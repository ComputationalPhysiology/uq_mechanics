...
eering. Springer 2007, New York.
    ...
    def __init__(self, pts, kernel, indx, indy=None, rkmax=32, eps=1.e-9, verbose=False):
        ...
        if verbose:
            print
            "Time to construct cluster tree is %g " % (time() - start)
    ...
    def mult(self, x, y, verbose=False):
        ...
        if verbose:
            print
            "Time for mat-vec is %g" % (time() - start)
...
if __name__ == '__main__':
    ...
    print
    "Memory usage in MB %g" % (Q._memoryusage())
    Q.mult(x, yh)
    ...
    if direct == True:
        ...
        yd = np.dot(mat, x)
        print
        "Error is %g" % (np.linalg.norm(yd - yh) / np.linalg.norm(yd))
