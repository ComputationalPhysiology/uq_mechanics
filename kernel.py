...
def _gaussian(R, l, s):
    r = R / l
    sigma = s
    return sigma * sigma * np.exp(-r * r / 2.)
...
class Matern:
    ...
    def __init__(self, p=0, l=1., s=1.):
        self.p, self.l, self.s = p, l, s
        if isinstance(p, int):
            self.kernel = lambda r: _maternpolykernel(r, self.p, self.l)
        elif p == np.inf:
            self.kernel = lambda r: _gaussian(r, self.l, self.s)
        else:
...
if __name__ == '__main__':
    ...
    k3 = Matern(p=2, l=l)
    plt.plot(r, (1. + np.sqrt(5) * r / l + 5. * r ** 2. / (3. * l ** 2.)) * np.exp(-np.sqrt(5) * r / l),
             label=r'True $\nu = 5/2$')
