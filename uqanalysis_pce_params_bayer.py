...
joint = cp.J(b_ff_dist, b_xx_dist, b_fx_dist, CC_dist, K_dist, fiber_rotation_epi_dist, fiber_rotation_end_dist,
...
# file names
fileroot = "UQ_PCE-PC" + "_o" + str(order) + "_qmc" + str(qmc_scheme) + "_nnodes" + str(nnodes) + "_rm" + str(
    rgm_rule) + \
...
for s in nodes.T:
    ...
    while pressure <= pressures[len(pressures) - 1]:
        ...
        if (i == 0) & (pressure == 0.0):
            ...
            #np.savetxt(txt_file, np.column_stack((qoi_names, np.array(qoi[0:7]).T.astype(np.object))),
            #           fmt=['%s', '%.8f'], delimiter="  ")
            txt_file.write("\n")
            txt_file.write("\n")
...
np.savetxt(txt_file, prodata, fmt=['%s', '%.8f', '%.8f', '%.8f', '%.8f', '%.8f', '%.8f'], delimiter="  ",
           header="qoi, mean, stdv, vari, cova, kurt, skew")
txt_file.write("\n")
txt_file.write("\n")
# np.savetxt(txt_file, metrics,fmt=['%.5f','%.5f', '%.5f','%.5f','%.5f','%.5f'], header = "mean, stdv, vari, cova, kurt, skew")
# saving qoi raw data
np.savetxt(hist_file, rawdata, fmt=['%.10f', '%.10f', '%.10f', '%.10f', '%.10f', '%.10f', '%.10f'], delimiter=" ",
           header="dVol, dLen, dRen, dRep, dThi, dTwi, dWV")
...
for i in range(len(qois)):
    ...
    df.to_csv(sai2_file, sep=' ', header=" b_ff, b_xx, b_fx, C, K, fr_epi,fr_endo, sr_epi, sr_endo",
              float_format='%.5f', index=True)
"""
