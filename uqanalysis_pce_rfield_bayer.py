...
"Order:", order
print
"Sampling Method:", qmc_scheme
print
"Number of nodes:", nnodes
print
"Regression method:", rgm_rule
print
"Correlaton Length:", l
print
"Sigma:", sigma
print
"Range of Pressures:", pressures
print
"---------------------------------"

# open files
fileroot = "UQ_PCE-PC" + "_o" + str(order) + "_qmc" + str(qmc_scheme) + "_nnodes" + str(nnodes) + "_rgm" + str(
    rgm_rule) + \
...
for s in nodes.T:
    ...
    while pressure <= pressures[len(pressures) - 1]:
        ...
        if (i == 0) & (pressure == 0.0):
            ...
            np.savetxt(txt_file, np.column_stack((qoi_names, np.array(qoi[0:9]).T.astype(np.object))),
                       fmt=['%s', '%.8f'], delimiter="  ")
            txt_file.write("\n")
            txt_file.write("\n")
...
for ts in [test_samples.T[ntests - 1]]:  # [test_samples.T[ntests -1]]: #test_samples.T[ntests-npoints-1: ntests -1]:
    ...
    while pressure <= pressures[len(pressures) - 1]:
        ...
        while np.isnan(qoi[0]) == True:
            nnan = nnan + 1
            print
            "WARNING  NaN in cross-validation:", nnan
